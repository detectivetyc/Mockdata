...
class FileLogThread(threading.Thread):
    def __init__(self, threadID, name, download_log_num, upload_log_num, table, buffer):
    ...
    def run(self):
        ...
        run_file_log(self.name, self.download_log_num, self.upload_log_num, self.table, self.buffer)
        print "Ending " + self.name

redis_instance = RedisConnection(ReadConfiguration.redis_port, ReadConfiguration.redis_host, ReadConfiguration.redis_db)
def run_file_log(threadName, download_log_num, upload_log_num, table, buffer):
